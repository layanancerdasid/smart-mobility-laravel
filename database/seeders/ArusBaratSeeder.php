...
class ArusBaratSeeder extends Seeder
{
    public function run(): void
    {
        $jsonPath = database_path('seeders/data/ARUS_LALU_LINTAS_BARAT.json');
        ...
        //     DB::table('arus_lalu_lintas_barat')->updateOrInsert($chunk->toArray());
        // });

        collect($data)->chunk(100)->each(function ($chunk) {
            foreach ($chunk as $item) {
                DB::table('arus_lalu_lintas_barat')->updateOrInsert(
                    [
                        'ID_Simpang' => $item['ID_Simpang'],
                        'Arah' => $item['Arah'],
                        'Waktu' => $item['Waktu']
